classdef VEXCompanionAppApp < handle
    %
    % Usage:
    %     app.VEXCompanionAppApp
    ...
    properties (Constant)
        AppPathBase = {'/VEXCompanionApp'};
        AppClass = 'VEXCompanionAppApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function obj = VEXCompanionAppApp()
        ...
        function startApp(obj)
            ...
            mlock;
            VEXCompanionAppApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'VEXCompanionApp')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong VEXCompanionApp
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'VEXCompanionApp').getString, ...
            ...
            if nargout(@VEXCompanionApp) == 0
                eval('VEXCompanionApp');
            else
                obj.AppHandle = eval('VEXCompanionApp');
            end
